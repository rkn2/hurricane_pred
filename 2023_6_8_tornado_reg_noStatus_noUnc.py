...
# Drop columns containing the word "unc"
columns_to_drop = [column for column in data.columns if 'unc' in column]
data = data.drop(columns=columns_to_drop)
...
automl = AutoML(
    results_path='2023_6_9_reg_noStatus_noUnc_noGolden_rand',
   ...
   #algorithms=["CatBoost", "Xgboost", "LightGBM", "Random Forest", "Linear", "Decision Tree"],
    ...
    mix_encoding=False, # only needed for cat
    validation_strategy={
